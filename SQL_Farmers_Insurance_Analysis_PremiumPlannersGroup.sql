...
having TotalPremiumAmount > 1000000000  
...
where SumInsured >
 ( 
select max(SumInsured) from farmerinsurancedata
where FarmersPremiumAmount =(
select max(FarmersPremiumAmount) from farmerinsurancedata)
) ;
-- ###
...
add constraint fk_states_new foreign key (StateCode) references States(StateCode);
